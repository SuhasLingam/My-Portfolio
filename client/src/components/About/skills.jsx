  ...
  return (
    <div className="md:flex-col md:items-center md:justify-center md:w-full md:h-screen sticky flex flex-col items-center justify-center">
      <h1 className="md:mb-[60px] md:text-6xl my-11 text-4xl font-anton text-white">
        My <b className="text-blue-500">Tech</b> Stack
      </h1>

      <div className="md:gap-9 gap-7 md:h-fit md:grid-cols-6 md:grid-rows-4 grid-rows-8 grid grid-flow-row grid-cols-3 p-5 text-white">
        {SkillIcons.map((i) => (
          <span className="md:w-[150px] w-[120px] h-[85px] rounded-2xl hover:transition-all hover:ease-in-out hover:delay-0 ring-2 hover:shadow-custom hover:shadow-blue-600 ring-white md:h-[90px] justify-center items-center flex md:p-4 md:text-3xl bg-blue-900 bg-opacity-[60%] ">
            {i.name}
          </span>
        ))}
      </div>
